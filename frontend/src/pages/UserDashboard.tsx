...
import { Users, ChevronRight, Droplets, Phone, IndianRupee, Plus } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-card rounded-3xl p-5 border border-border shadow-sm col-span-2 lg:col-span-1">
          ...
          <p className="font-bold text-2xl text-foreground">
            {customers.length > 0 ? `₹${(customers.reduce((s, c) => s + c.pricePerLitre, 0) / customers.length).toFixed(0)}` : "—"}
            <span className="text-base text-muted-foreground font-medium">/L</span>
          </p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-3xl border border-border shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          <h3 className="font-bold text-foreground">My Customers</h3>
          <Link href="/customers">
